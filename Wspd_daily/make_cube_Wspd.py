...
def main():
    ...
    data_files_8n = os.listdir('/home/michael/Desktop/Wspd_daily/8')
    data_files_5n = os.listdir('/home/michael/Desktop/Wspd_daily/5')
    data_files_2n = os.listdir('/home/michael/Desktop/Wspd_daily/2')
    data_files_0n = os.listdir('/home/michael/Desktop/Wspd_daily/0')  
    data_files_2s = os.listdir('/home/michael/Desktop/Wspd_daily/-2')
    data_files_5s = os.listdir('/home/michael/Desktop/Wspd_daily/-5')
    data_files_8s = os.listdir('/home/michael/Desktop/Wspd_daily/-8')
    ...
    for index_lat in xrange(len(all_files)):
        ...
        for index_lon in xrange(len(all_files[index_lat])):
            ...
            raw_data, anomoly_data, monthly_average_data, time_points = extract_data(latitude, longitude, masked_value)
    ...
    raw_data_cube.rename('Wind speed')
    raw_data_cube.convert_units('m s-1')
    print 'raw done'
    
    anomoly_cube = anomoly_data_cube_list_2D.merge()[0]
    anomoly_cube.rename('Wind anomoly')
    print 'anomoly done'
   
    
    monthly_average_cube = monthly_average_cube_list_2D.merge()[0]
    monthly_average_cube.rename('Average Wind Speed by month')
    print 'monthly average done'
    
    average_temp_cube = raw_data_cube.collapsed('time', iris.analysis.MEAN)
    average_temp_cube.rename('Average Wind Speed')
    print 'average temp done'
    
    all_cubes = [raw_data_cube, monthly_average_cube, anomoly_cube, average_temp_cube]
    
    file_type = '.nc'
    filename = 'Wspd_cubes' + file_type
    
    iris.save(all_cubes, filename)
    print 'finished'
    
    
def extract_data(latitude, longitude, masked_value):
    ...
    filename = '/home/michael/Desktop/Wspd_daily/' + str(latitude) + '/' + str(longitude) + '.txt'
    ...
    anomoly_data, monthly_average_data = convert_to_anomoly(raw_data, masked_value, tolerance)
    ...
    return raw_data, anomoly_data, monthly_average_data, time_points
...
def convert_to_anomoly(data_array, masked_value, tolerance):
    ...
    for (index, data_point) in enumerate(data_array):
        for counter in xrange(12):
            if (index+counter)%12 == 0:
                ...
                average_data.append(value)             
    
    average_array = np.array(average_data)
    average_array = np.ma.masked_values(average_array, masked_value, atol=tolerance)
    ...
    return average_array, monthly_average_array
